	...
	public void screenDraw(Graphics2D g) {
		...
		//노트가 출력 되는곳
				for(int i=0; i < noteList.size(); i++) {
					...
					if(!note.isProceeded()) { //사용되지않는 노트는 지워주기
						noteList.remove(i);
						i--;
					}else {
						note.screenDraw(g);
		...
		g.drawImage(judgeImage, 460, 470, null);
	...
	public void releasedL() {
		...
		public void dropNotes(String titleName) {
			...
			if(titleName.equals("Joakim Karud - Mighty Love") && difficulty.equals("Easy")) {
				int startTime = 4450 - dynamic_beat_14.Main.REACH_TIME * 1000;// 약 4초
			...
			int i = 0;
			gameMusic.start();//여기로 옮긴이유는 배열이 초기화하는 시간이 걸리기때문에
		...
		//판정함수
		public void judge(String input) {//Queue특성(먼저 들어온것부터 처리)을 살릴 수 있음
			for(int i=0; i < noteList.size(); i++) {
				Note note = noteList.get(i);
				if(input.equals(note.getNoteType())) {//입력값이 notetype가 일치한다면 s입력했는데 notetype이 s라면?
					judgeEvent(note.judge());//해당 노트의 판정을 눌러와서그것을 받아서 헤당하는 이미지 처리
					break;
		...
		public void judgeEvent(String judge) {
			if(!judge.equals("None")) {//judge이 있는 경우
